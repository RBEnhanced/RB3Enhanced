...
char *LocalizeHook(int thisLocale, Symbol sym, int fail)
{
    ...
    // TODO(Emma): allow a mod to replace this and include its own version
    ...
    for (i = 0; i < numOverrideLocales; i++)
    {
        if (strcmp(sym.sym, overrideLocales[i][0]) == 0)
        {
            ...
            // TODO(Emma): this doesn't work, the original Locale::FindDataIndex doesn't seem to like it when
            // sym.sym isn't in the same table as everything else...
            sprintf(newLocaleName, "rb3e_%s", sym.sym);
            sym.sym = newLocaleName;
