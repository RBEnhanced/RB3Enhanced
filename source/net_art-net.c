...
void RB3E_InitArtNet()
{
    ...
    memset(ArtNetPacket.Data, 0, 512 * sizeof(ArtNetPacket.Data[0]));
...
{    
    ...
    else if ( (state2 && 0xE0) != 0x00) // Led colour state
    {
        ...
        for (; led_number<8; led_number++)
        {
            ...
            {
                ArtNetPacket.Data[config.ArtNetColourTranslation[colour_id][led_number][0]] = 0;
            }
        ...
        {  // Additive.  All values for 'on' led colours for same channel are added together.
            ...
            for (led_number = 0; led_number<8; led_number++)
            {
                if( (state1 & led_id) == led_id )
                {
                    ArtNetPacket.Data[config.ArtNetColourTranslation[colour_id][led_number][0]] += config.ArtNetColourTranslation[colour_id][led_number][1];
                }
